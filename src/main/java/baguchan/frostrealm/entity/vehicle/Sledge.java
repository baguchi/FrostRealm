...
public class Sledge extends Boat {
    ...
    public Sledge(Level p_38293_, double p_38294_, double p_38295_, double p_38296_) {
        this(FrostEntities.SLEDGE.get(), p_38293_);
    ...
    public ItemStack getPickResult() {
        return FrostItems.OAK_SLEDGE.get().getDefaultInstance();
    }
        ...
        if (this.isVehicle()) {
            ...
            for (FrostWolf wolf : frostWolfList) {
                if (wolf.isTame() && !wolf.isInSittingPose()) {
                    wolf.getMoveControl().setWantedPosition(wolf.getX() + this.getLookAngle().x * 4, wolf.getY() + this.getLookAngle().y * 4, wolf.getZ() + this.getLookAngle().z * 4, 1.3F);
                }
