...
public class FrostBoarAi {
    ...
    public static void updateActivity(GlacierBoar boar) {
        Brain<GlacierBoar> brain = boar.getBrain();
    ...
    private static RunOne<GlacierBoar> createIdleMovementBehaviors() {
    ...
    private static boolean wantsToStopFleeing(GlacierBoar p_34618_) {
        return p_34618_.isAdult() && isEnoughFrostBoarAndHealth(p_34618_);
    }

    private static boolean isEnoughFrostBoarAndHealth(GlacierBoar p_34623_) {
        if (p_34623_.isBaby()) {
            return false;
        } else {
    ...
    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(ServerLevel serverLevel, GlacierBoar p_34611_) {
    ...
    private static void retreatFromNearestTarget(GlacierBoar p_34613_, LivingEntity p_34614_) {
        Brain<GlacierBoar> brain = p_34613_.getBrain();
    ...
    private static void setAvoidTarget(GlacierBoar p_34620_, LivingEntity p_34621_) {
        p_34620_.getBrain().eraseMemory(MemoryModuleType.ATTACK_TARGET);
        p_34620_.getBrain().eraseMemory(MemoryModuleType.WALK_TARGET);
        p_34620_.getBrain().setMemoryWithExpiry(MemoryModuleType.AVOID_TARGET, p_34621_, (long) RETREAT_DURATION.sample(p_34620_.level().getRandom()));
    }


    public static void wasHurtBy(ServerLevel serverLevel, GlacierBoar p_34596_, LivingEntity p_34597_) {
        if (!(p_34597_ instanceof GlacierBoar)) {
            Brain<GlacierBoar> brain = p_34596_.getBrain();
    ...
    private static void maybeRetaliate(ServerLevel serverLevel, GlacierBoar p_34625_, LivingEntity p_34626_) {
        if (!p_34625_.getBrain().isActive(Activity.AVOID) || p_34626_.getType() != FrostEntities.YETI.get()) {
            if (p_34626_.getType() != FrostEntities.GLACIER_BOAR.get()) {
    ...
    private static void setAttackTarget(GlacierBoar p_34630_, LivingEntity p_34631_) {
        Brain<GlacierBoar> brain = p_34630_.getBrain();
        brain.eraseMemory(MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE);
        brain.eraseMemory(MemoryModuleType.BREED_TARGET);
        brain.setMemoryWithExpiry(MemoryModuleType.ATTACK_TARGET, p_34631_, 200L);
    }

    public static void onHitTarget(GlacierBoar p_34580_, LivingEntity p_34581_) {
        if (!p_34580_.isBaby()) {
            broadcastAttackTarget(p_34580_, p_34581_);
        }
    }

    private static void broadcastAttackTarget(GlacierBoar p_34635_, LivingEntity p_34636_) {
        getVisibleAdultFrostBoars(p_34635_).forEach((p_34574_) -> {
            setAttackTargetIfCloserThanCurrent(p_34574_, p_34636_);
        });
    }

    public static List<GlacierBoar> getVisibleAdultFrostBoars(LivingEntity p_34874_) {
        return p_34874_.getBrain().getMemory(FrostMemoryModuleType.NEAREST_FROST_BOARS.get()).orElse(ImmutableList.of());
    }

    private static void setAttackTargetIfCloserThanCurrent(GlacierBoar p_34640_, LivingEntity p_34641_) {
    ...
    public static Optional<SoundEvent> getSoundForCurrentActivity(GlacierBoar boar) {
        return boar.getBrain().getActiveNonCoreActivity().map((activity) -> {
            return getSoundForActivity(boar, activity);
        });
    }

    private static SoundEvent getSoundForActivity(GlacierBoar p_34583_, Activity p_34584_) {
