...
import baguchan.frostrealm.client.model.ChestRaftSledgeModel;
import baguchan.frostrealm.client.model.ChestSledgeModel;
import baguchan.frostrealm.client.model.RaftSledgeModel;
import baguchan.frostrealm.client.model.SledgeModel;
...
@OnlyIn(Dist.CLIENT)
public class SledgeRenderer<T extends Boat> extends EntityRenderer<T> {
    private final Map<Boat.Type, Pair<ResourceLocation, ListModel<Boat>>> boatResources;

    public SledgeRenderer(EntityRendererProvider.Context p_234563_, boolean p_234564_) {
        super(p_234563_);
    ...
    private ListModel<Boat> createBoatModel(EntityRendererProvider.Context p_248834_, Boat.Type p_249317_, boolean p_250093_) {
        ModelLayerLocation modellayerlocation = p_250093_ ? ClientRegistrar.createChestSledgeModelName(p_249317_) : ClientRegistrar.createSledgeModelName(p_249317_);
        ModelPart modelpart = p_248834_.bakeLayer(modellayerlocation);
        if (p_249317_ == Boat.Type.BAMBOO) {
            return (ListModel<Boat>) (p_250093_ ? new ChestRaftSledgeModel(modelpart) : new RaftSledgeModel(modelpart));
        } else {
            return (ListModel<Boat>) (p_250093_ ? new ChestSledgeModel(modelpart) : new SledgeModel(modelpart));
        }
    }
